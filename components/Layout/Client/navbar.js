...
<a href="/" target="_blank" rel="noopener noreferrer" previewlistener="true">BEAUTY BONANZA Get Your Amazing Deals!</a>
        ...
        <div className={c.rightDiv}>
            <ul>
                <li>
                    <a href="/" target="_blank" rel="noopener noreferrer" previewlistener="true">
                        <span class="ts-icon">
        ...
        <div className={c.container}>
          ...
          <div className={c.end}>
            <Link href={"/signin"} passHref legacyBehavior>
              ...
              {!session && (
                  <Link href="/signin">
                    <button className="btn btn-md btn-danger rounded-4 mb-2 ml-2">Sign In</button>
                  </Link>
                )}
                {session && (
                  // <Link href="/profile">
                  ...
                  <button className="btn btn-danger dropdown-toggle mb-2 rounded border-radius-5" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                  <Person width={24} height={24} className="dropdown-toggle"/>
                  </button>
                  <ul className="dropdown-menu">
                    <li><Link className="dropdown-item" href="/profile">Profile</Link></li>
                   
                    <li><hr className="dropdown-divider"/></li>
                    <li><span onClick={() => signOut({ callbackUrl: "/" })} className="dropdown-item" href="/signout">Logout</span></li>
                  </ul>
                </div>
                )}
    ...
    <div className={c.bottom_bar}>
          ...
          {cat.map((item, idx) => (
            ...
            <ul className="dropdown-menu shadow-sm" style={{width:'250px'}}>
               {item.subCategories.map((subItem, subIdx) => (

              <li key={subIdx} className={`${c.subContentHeading} p-1 dropdown-item`}>
                              <Link
                            ...
                            <ul>
                              {subItem.child.map((childItem, childIdx) => (
                                <li key={childIdx} className="dropdown-item p-3">
                                  <Link
